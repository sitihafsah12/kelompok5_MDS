...
for (i in 1:nrow(data_film)) {
  ...
  id_aktor <- ifelse(data_film[i, "id_aktor"] == "", "NULL", data_film[i, "id_aktor"])
  
  query <- paste0("INSERT INTO \"film\" (id_film, Id_sutradara, id_aktor, judul_film, tahun_rilis, durasi_film, genre_film, votes, rating) VALUES (",
                  "'", data_film[i, "id_film"], "', ",
                  id_sutradara, ", ",
                  "'", data_film[i, "id_aktor"], "', ",
...
# Data aktor
data_aktor <- read.csv("https://raw.githubusercontent.com/sitihafsah12/kelompok5_MDS/main/Data/aktor.csv", sep = ",", fill = TRUE)
str(data_aktor)
for (i in 1:nrow(data_aktor)) {
  # Check if Id_Aktor is not an empty string
  if (data_aktor[i, "id_aktor"] != "") {
    query <- paste0("INSERT INTO \"aktor\" (id_film, id_aktor, nama_aktor) VALUES (",
                    "'", data_aktor[i, "id_film"], "', ",
                    "'", data_aktor[i, "id_aktor"], "', ",
                    "'", data_aktor[i, "nama_aktor"], "');")
    query_execute <- DBI::dbGetQuery(conn = DB, statement = query)
  }
}
